...
static int my_utf8_main(int argc, char ** argv)
{
    int i, ret, insertspace, alpha0same;

    insertspace = 0;
    alpha0same = 0;

    /* check for opts --space and --alpha and adjust argv0, argc and argv to skip them */
    while(argc >= 2 && (samestring(argv[1], "--space") || samestring(argv[1], "--alpha")))
    {
        if(samestring(argv[1], "--space"))
            insertspace = 1;

        if(samestring(argv[1], "--alpha"))
            alpha0same = 1;

        argv[1] = argv[0];
        ++argv;
        --argc;
    } /* while --space or --alpha opts */
